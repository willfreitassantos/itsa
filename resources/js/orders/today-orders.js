$(document).on('submit', '#formTodayOrders', function(evt) {
	evt.preventDefault();
	$.ajax({
		type: 'POST',
		url: '../orders/today/filter',
		data: {
			'searchParam': $('#today-orders-search-param').val().trim()
		},
		dataType: 'json',
		success: function(json) {
			...
			$('#list-today-orders tbody tr').not('.table-heading').remove();
			$('#list-today-orders tbody').append(result);
		},
		error: function(json) {
